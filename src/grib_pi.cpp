...
int grib_pi::Init(void) {
  ...
  m_bBundledGribConflict = IsBundledGribPluginEnabled(m_pconfig);
  if (m_bBundledGribConflict) {
    wxLogWarning(
        "xGRIB: bundled GRIB is enabled; xGRIB viewer and overlays will remain "
        "inactive until bundled GRIB is disabled and OpenCPN is restarted.");
  }
  ...
  if (m_bGRIBShowIcon) {
    ...
    m_leftclick_tool_id = InsertPlugInToolSVG(
        "", normalIcon, rolloverIcon, toggledIcon, wxITEM_CHECK, _("xGRIB"), "",
  ...
  if (openSmokeFile || openSmokeGenerator) {
    wxTheApp->CallAfter(
      ...
      if (m_pGribCtrlBar && openSmokeFile) {
        m_pGribCtrlBar->OpenGeneratedGrib(smokeTestFile);
        wxLogMessage("xGRIB smoke test opened: %s", smokeTestFile);
      }
      if (m_pGribCtrlBar && openSmokeGenerator) {
        m_pGribCtrlBar->ShowEnvironmentalGenerator();
        wxLogMessage("xGRIB smoke test opened environmental generator dialog");
      }
    });
  }
...
wxString grib_pi::GetShortDescription() {
  return _("xGRIB display and environmental GRIB generator for OpenCPN");
}

wxString grib_pi::GetLongDescription() {
  return _(
      "xGRIB PlugIn for OpenCPN\n\
...
void grib_pi::ShowPreferencesDialog(wxWindow *parent) {
  ...
  if (Pref->m_textDirectory) {  // not present on Android
    ...
    if (pConf) {
      ...
      pConf->Read(_T ( "xGRIBDirectory" ), &l_grib_dir);
...
void grib_pi::UpdatePrefs(GribPreferencesDialog *Pref) {
  ...
  if (Pref->m_grib_dir_sel.Length()) {
    ...
    if (pConf) {
      ...
      pConf->DeleteGroup(_T ( "/Settings/xGRIB/FileNames" ));
      pConf->Flush();
    }
  }
  SaveConfig();
...
void grib_pi::OnToolbarToolCallback(int id) {
  ...
  if (m_bBundledGribConflict) {
    if (id >= 0 && !m_bConflictWarningShown) {
      m_bConflictWarningShown = true;
      wxMessageBox(
          _("xGRIB and the bundled GRIB plugin cannot be active together.\n\n"
            "Disable bundled GRIB in Settings > Plugins, enable xGRIB, then "
            "restart OpenCPN."),
          _("xGRIB is inactive"), wxOK | wxICON_WARNING, m_parent_window);
    }
    if (m_leftclick_tool_id >= 0)
...
bool grib_pi::LoadConfig(void) {
  ...
  pConf->SetPath(_T( "/PlugIns/xGRIB" ));
