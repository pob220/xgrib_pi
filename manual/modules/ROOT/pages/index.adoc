...
Waves and Current have related but distinct symbol controls. A Particle Map is
available for currents. See the description above for wind Particle Maps.
Once again different units. +

Wave symbols can be displayed as a travel-direction arrow, double arrow,
height-proportional arrow, wave crests with a travel marker, or a height circle
with a direction tick. Symbol size, colour and fixed or minimum spacing can be
set independently. Wave directions are source bearings; the travel marker
points in the opposite, propagation direction. +

Currents can use single, double or proportional arrows. Single and double
arrows have a fixed configured size and show direction. Proportional arrows
grow with speed; both the baseline size and the growth in pixels per knot are
configurable. Current arrow colour and spacing can also be selected. +
...
* WVDIR Wind Waves Direction (source bearing; symbols point toward travel) [109]
